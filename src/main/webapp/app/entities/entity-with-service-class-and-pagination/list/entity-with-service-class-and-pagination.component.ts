...
export class EntityWithServiceClassAndPaginationComponent implements OnInit {
  ...
  trackId(index: number, item: IEntityWithServiceClassAndPagination): string {
    return item.id!;
  }

  delete(entityWithServiceClassAndPagination: IEntityWithServiceClassAndPagination): void {
    const modalRef = this.modalService.open(EntityWithServiceClassAndPaginationDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: IEntityWithServiceClassAndPagination[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
