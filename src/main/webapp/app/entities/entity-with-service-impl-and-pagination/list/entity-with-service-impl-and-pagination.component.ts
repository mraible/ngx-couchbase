...
export class EntityWithServiceImplAndPaginationComponent implements OnInit {
  ...
  trackId(index: number, item: IEntityWithServiceImplAndPagination): string {
    return item.id!;
  }

  delete(entityWithServiceImplAndPagination: IEntityWithServiceImplAndPagination): void {
    const modalRef = this.modalService.open(EntityWithServiceImplAndPaginationDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: IEntityWithServiceImplAndPagination[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
