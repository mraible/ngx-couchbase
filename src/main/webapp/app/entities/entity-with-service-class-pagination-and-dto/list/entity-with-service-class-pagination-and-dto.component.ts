...
export class EntityWithServiceClassPaginationAndDTOComponent implements OnInit {
  ...
  trackId(index: number, item: IEntityWithServiceClassPaginationAndDTO): string {
    return item.id!;
  }

  delete(entityWithServiceClassPaginationAndDTO: IEntityWithServiceClassPaginationAndDTO): void {
    const modalRef = this.modalService.open(EntityWithServiceClassPaginationAndDTODeleteDialogComponent, {
      size: 'lg',
      backdrop: 'static',
    });
  ...
  protected onSuccess(data: IEntityWithServiceClassPaginationAndDTO[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
