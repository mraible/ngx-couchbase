...
export class EntityWithServiceImplPaginationAndDTORoutingResolveService implements Resolve<IEntityWithServiceImplPaginationAndDTO> {
  constructor(protected service: EntityWithServiceImplPaginationAndDTOService, protected router: Router) {}

  resolve(route: ActivatedRouteSnapshot): Observable<IEntityWithServiceImplPaginationAndDTO> | Observable<never> {
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        mergeMap((entityWithServiceImplPaginationAndDTO: HttpResponse<EntityWithServiceImplPaginationAndDTO>) => {
