...
export class EntityWithPaginationAndDTOComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.entityWithPaginationAndDTOService
      ...
      .subscribe(
        (res: HttpResponse<IEntityWithPaginationAndDTO[]>) => {
  ...
  trackId(index: number, item: IEntityWithPaginationAndDTO): string {
    return item.id!;
  }

  delete(entityWithPaginationAndDTO: IEntityWithPaginationAndDTO): void {
    const modalRef = this.modalService.open(EntityWithPaginationAndDTODeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: IEntityWithPaginationAndDTO[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
