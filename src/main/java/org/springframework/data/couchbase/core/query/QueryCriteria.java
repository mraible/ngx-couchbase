...
/**
 *
 * TODO: This needs to be fixed in Spring Data.
 *  There is no easy way to extend this class so I had to put it here so it can be loaded instead of the one defined in the JAR.
 *  Uncomment only if we need support for find by id using meta() in Criteria. In other words if block in
 *  CustomN1qlQueryCreator.java#45 is not commented out
 *
